...
const addAsset = async (req, res) => {
  try {
    const { symbol, name, amount, purchasePrice, currentPrice, date, exchange, notes, priceChangeThreshold } = req.body;

    // Validate required fields
    if (!symbol || !name || !amount || !purchasePrice || !date || !exchange) {
    ...
    const newAsset = new Asset({
      ...
      priceChangeThreshold: priceChangeThreshold ? parseFloat(priceChangeThreshold) : 1,
    });
...
const getAllAssets = async (req, res) => {
  try {
    const assets = await Asset.find({ userId: req.user._id.toString() }); // Filter by userId (string)
...
const getAssetById = async (req, res) => {
  try {
    const { id } = req.params;
    const asset = await Asset.findOne({ id, userId: req.user._id.toString() }); // Filter by userId (string)
...
const updateAsset = async (req, res) => {
  try {
    ...
    if (updateData.priceChangeThreshold) {
      updateData.priceChangeThreshold = parseFloat(updateData.priceChangeThreshold);
    }

    const updatedAsset = await Asset.findOneAndUpdate(
      { id, userId: req.user._id.toString() }, // Filter by userId (string)
      updateData,
...
const deleteAsset = async (req, res) => {
  try {
    const { id } = req.params;
    const deletedAsset = await Asset.findOneAndDelete({ id, userId: req.user._id.toString() }); // Filter by userId (string)
